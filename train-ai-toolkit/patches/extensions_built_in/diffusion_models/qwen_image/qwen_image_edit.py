...
class QwenImageEditModel(QwenImageModel):
    ...
    def condition_noisy_latents(
        ...
        with torch.no_grad():
            # use cached control latents if available (set during cache_latents_all_latents)
            if (
                batch.file_items is not None
                and len(batch.file_items) > 0
                and batch.file_items[0].is_latent_cached
                and batch.file_items[0]._cached_control_latent is not None
            ):
                control_latents = torch.stack(
                    [fi._cached_control_latent for fi in batch.file_items]
