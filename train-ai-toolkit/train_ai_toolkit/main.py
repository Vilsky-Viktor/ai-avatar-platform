...
import train_ai_toolkit.logger as log
from train_ai_toolkit.logger import set_job_id, clear_job_id
import train_ai_toolkit.db as db
import train_ai_toolkit.message_queue as mq
import train_ai_toolkit.storage as storage
import train_ai_toolkit.training as training
from train_ai_toolkit.models import Job
...
def make_process_job(semaphore: threading.Semaphore):
    def process_job(message: pubsub_v1.subscriber.message.Message):
        ...
        try:
            ...
            if not training_cfg.toolkit.config.process:
            ...
            process = training_cfg.toolkit.config.process[0]
            lr = process.train.lr
            lora_rank = process.network.linear
            resolution = process.datasets[0].resolution[0]
            arch = process.model.arch
            ...
            try:
                ...
                training.run_training(training_cfg.toolkit.model_dump(), out_dir, images_dir, control_dir, training_cfg.modelName)
            except Exception as e:
...
def main():
    logger.info(f"Starting train-ai-toolkit service (concurrency={MESSAGE_CONCURRENCY})")
