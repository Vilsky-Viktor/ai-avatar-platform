...
from train_ai_toolkit.logger import logger

TRAIN_AI_TOOLKIT_PATH = os.environ.get("TRAIN_TRAIN_AI_TOOLKIT_PATH", "/opt/train-ai-toolkit")
...
def run_training(toolkit_config: dict, out_dir: Path, images_dir: Path, control_dir: Path, model_name: str):
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "train_ai_toolkit.training", config_path],
            check=False,
        )
...
if __name__ == "__main__":
    if TRAIN_AI_TOOLKIT_PATH not in sys.path:
        sys.path.insert(0, TRAIN_AI_TOOLKIT_PATH)
