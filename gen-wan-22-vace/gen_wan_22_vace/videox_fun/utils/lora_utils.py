...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        _logger.debug(f"create LoRA network. base dim (rank): {lora_dim}, alpha: {alpha}")
        _logger.debug(f"neuron dropout: p={self.dropout}")
        ...
        for i, text_encoder in enumerate(text_encoders):
            if text_encoder is not None:
                ...
                skipped_te += skipped
        _logger.debug(f"create LoRA for Text Encoder: {len(self.text_encoder_loras)} modules.")

        self.unet_loras, skipped_un = create_modules(True, unet, LoRANetwork.TRANSFORMER_TARGET_REPLACE_MODULE)
        _logger.debug(f"create LoRA for U-Net: {len(self.unet_loras)} modules.")
    ...
    def apply_to(self, text_encoder, unet, apply_text_encoder=True, apply_unet=True):
        if apply_text_encoder:
            _logger.debug("enable LoRA for text encoder")
        else:
            self.text_encoder_loras = []

        if apply_unet:
            _logger.debug("enable LoRA for U-Net")
        else:
            self.unet_loras = []
...
def merge_lora(pipeline, lora_path, multiplier, device='cpu', dtype=torch.float32, state_dict=None, transformer_only=False, sub_transformer_name="transformer"):
    ...
    _logger.debug(f"[LoRA Merge] Starting to merge LoRA from: {lora_path if lora_path else 'state_dict'}")
    _logger.debug(f"[LoRA Merge] Multiplier: {multiplier}, Device: {device}, Dtype: {dtype}")
    ...
    if state_dict is None:
        if lora_path.endswith("safetensors"):
            _logger.debug(f"[LoRA Merge] Loading safetensors file...")
            state_dict = load_file(lora_path)
        else:
            _logger.debug(f"[LoRA Merge] Loading pytorch file...")
            state_dict = torch.load(lora_path, map_location="cpu")
    else:
        _logger.debug(f"[LoRA Merge] Using provided state_dict")
    ...
    _logger.debug(f"[LoRA Merge] Organized into {len(updates)} layers")

    sequential_cpu_offload_flag = False
    if pipeline.transformer.device == torch.device(type="meta"):
        _logger.debug(f"[LoRA Merge] Removing hooks for meta device...")
                ...
                while len(layer_infos) > -1:
                    try:
                        ...
                        try:
                            ...
                            if len(layer_infos) == 0:
                                _logger.debug(f'[LoRA Merge] Warning: Error loading layer in front search: {layer}. Try it in back search.')
                            if len(temp_name) > 0:
                ...
                while start_index < len_layer_infos:
                    try:
                        if start_index >= end_indx:
                            _logger.debug(f'[LoRA Merge] Error: Failed to load layer in back search: {layer}')
                            error_flag = True
                            break
    ...
    _logger.info(f"[LoRA Merge] Completed: {merged_count} layers merged, {skipped_count} layers skipped, {error_count} errors")

    if sequential_cpu_offload_flag:
        _logger.debug(f"[LoRA Merge] Re-enabling sequential CPU offload...")
        ...
        try:
            ...
            if _is_group_offload_enabled(local_transformer):
                _logger.debug(f"[LoRA Merge] Removing group offload hooks from pipeline...")
                safe_remove_group_offloading(pipeline)
                _logger.debug(f"[LoRA Merge] Re-applying group offload hooks to pipeline...")
                ...
                _logger.debug(pipeline._execution_device)
        except Exception as e:
            _logger.debug(f"[LoRA Merge] Warning: Failed to refresh group offload: {e}")
    _logger.debug(f"[LoRA Merge] ✓ LoRA merge finished successfully")
    return pipeline
...
def unmerge_lora(pipeline, lora_path, multiplier=1, device="cpu", dtype=torch.float32, sub_transformer_name="transformer"):
    ...
    _logger.debug(f"[LoRA Unmerge] Starting to unmerge LoRA from: {lora_path}")
    _logger.debug(f"[LoRA Unmerge] Multiplier: {multiplier}, Device: {device}, Dtype: {dtype}")
    ...
    if lora_path.endswith("safetensors"):
        _logger.debug(f"[LoRA Unmerge] Loading safetensors file...")
        state_dict = load_file(lora_path)
    else:
        _logger.debug(f"[LoRA Unmerge] Loading pytorch file...")
        state_dict = torch.load(lora_path, map_location="cpu")
    ...
    _logger.debug(f"[LoRA Unmerge] Organized into {len(updates)} layers")

    sequential_cpu_offload_flag = False
    if pipeline.transformer.device == torch.device(type="meta"):
        _logger.debug(f"[LoRA Unmerge] Removing hooks for meta device...")
        pipeline.remove_all_hooks()
        sequential_cpu_offload_flag = True
        ...
        try:
            ...
            try:
                while len(layer_infos) > -1:
                    try:
                        ...
                        try:
                            ...
                            if len(layer_infos) == 0:
                                _logger.debug(f'[LoRA Unmerge] Warning: Error loading layer in front search: {layer}. Try it in back search.')
                            if len(temp_name) > 0:
                                temp_name += "_" + layer_infos.pop(0)
                            else:
                                temp_name = layer_infos.pop(0)
                ...
                while start_index < len_layer_infos:
                    try:
                        if start_index >= end_indx:
                            _logger.debug(f'[LoRA Unmerge] Error: Failed to load layer in back search: {layer}')
                            error_flag = True
                            break
                        curr_layer = curr_layer.__getattr__("_".join(layer_infos[start_index:end_indx]))
                        start_index = end_indx
    ...
    _logger.debug(f"[LoRA Unmerge] Completed: {unmerged_count} layers unmerged, {error_count} errors")

    if sequential_cpu_offload_flag:
        _logger.debug(f"[LoRA Unmerge] Re-enabling sequential CPU offload...")
        ...
        try:
            local_transformer = getattr(pipeline, sub_transformer_name)
            if _is_group_offload_enabled(local_transformer):
                _logger.debug(f"[LoRA Unmerge] Removing group offload hooks from pipeline...")
                safe_remove_group_offloading(pipeline)
                _logger.debug(f"[LoRA Unmerge] Re-applying group offload hooks to pipeline...")
                register_auto_device_hook(getattr(pipeline, sub_transformer_name))
                safe_enable_group_offload(
                    pipeline,
                    onload_device=device, offload_device="cpu", offload_type="leaf_level", use_stream=True
            ...
            _logger.debug(f"[LoRA Unmerge] Warning: Failed to refresh group offload: {e}")

    _logger.debug(f"[LoRA Unmerge] ✓ LoRA unmerge finished successfully")
    return pipeline
