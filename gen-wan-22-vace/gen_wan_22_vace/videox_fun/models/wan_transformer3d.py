...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_pretrained(
        ...
        _logger.debug(f"loaded 3D transformer's pretrained weights from {pretrained_model_path} ...")
        ...
        if low_cpu_mem_usage:
            try:
                ...
                if os.path.exists(model_file):
                    ...
                    state_dict = {}
                    _logger.debug(model_files_safetensors)
                ...
                for key in state_dict:
                    if key in model.state_dict() and model.state_dict()[key].size() == state_dict[key].size():
                        ...
                        _logger.debug(f"Skipping key '{key}' due to size mismatch or absence in model.")
                ...
                if missing_keys:
                    _logger.debug(f"Missing keys will be initialized: {sorted(missing_keys)}")
        ...
        for key in state_dict:
            if key in model.state_dict().keys() and model.state_dict()[key].size() == state_dict[key].size():
                ...
                _logger.debug(key, "Size don't match, skip")
                
        state_dict = tmp_state_dict

        m, u = model.load_state_dict(state_dict, strict=False)
        _logger.debug(f"### missing keys: {len(m)}; \n### unexpected keys: {len(u)};")
        _logger.debug(m)
        
        params = [p.numel() if "." in n else 0 for n, p in model.named_parameters()]
        _logger.debug(f"### All Parameters: {sum(params) / 1e6} M")

        params = [p.numel() if "attn1." in n else 0 for n, p in model.named_parameters()]
        _logger.debug(f"### attn1 Parameters: {sum(params) / 1e6} M")
