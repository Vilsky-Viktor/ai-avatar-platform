...
import gen_wan_22_vace.logger as log
from gen_wan_22_vace.logger import set_job_id, unset_job_id
import gen_wan_22_vace.db as db
import gen_wan_22_vace.message_queue as mq
import gen_wan_22_vace.storage as storage
from gen_wan_22_vace.models import Job
from gen_wan_22_vace import inference
...
def make_process_job(semaphore: threading.Semaphore):
    def process_job(message: pubsub_v1.subscriber.message.Message):
        ...
        try:
            ...
            job_input = job.input
            set_job_id(job.id)
            logger.info(f"Received VACE job: avatar={job.avatarId} mediaPaths={job_input.inference.mediaPaths}")
            ...
            # Ack early — generation takes many minutes
            message.ack()
...
def main():
    logger.info(f"Starting gen-wan-22-vace service (concurrency={MESSAGE_CONCURRENCY})")
