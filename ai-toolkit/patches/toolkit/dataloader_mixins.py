...
class LatentCachingFileItemDTOMixin:
    ...
    def get_control_latent(self, device=None):
        if not self.is_latent_cached:
            return None
        if self._cached_control_latent is None:
            state_dict = load_file(self.get_latent_path(), device='cpu')
            if 'control_latent' in state_dict:
                self._cached_control_latent = state_dict['control_latent']
        return self._cached_control_latent
            ...
            for file_item in tqdm(self.file_list, desc=f'Caching latents{" to disk" if to_disk else ""}'):
                ...
                if os.path.exists(latent_path):
                    ...
                    if hasattr(file_item, 'control_path') and file_item.control_path is not None:
                        ...
                        if file_item.control_tensor is not None:
                            ctrl = (file_item.control_tensor * 2 - 1).unsqueeze(0).to(device, dtype=dtype)
