...
from .videox_fun.pipeline import Wan2_2FunInpaintPipeline, Wan2_2FunControlPipeline
...
# Model names on disk (under LOCAL_MODELS_PATH)
INP_MODEL_NAME = os.environ.get("INP_MODEL_NAME", "wan22-a14b-fun-inp")
CTRL_MODEL_NAME = os.environ.get("CTRL_MODEL_NAME", "wan22-a14b-fun-ctrl")

GPU_MEMORY_MODE = os.environ.get("GPU_MEMORY_MODE", "sequential_cpu_offload")
WEIGHT_DTYPE = torch.bfloat16

VIDEO_LENGTH = int(os.environ.get("VIDEO_LENGTH", "81"))
...
_pipeline_inp: Optional[Wan2_2FunInpaintPipeline] = None
_pipeline_ctrl: Optional[Wan2_2FunControlPipeline] = None
_vae_inp = None
_vae_ctrl = None
_config_inp = None
_config_ctrl = None
...
def _load_config(model_path: str):
    ...
    if not os.path.exists(config_path):
        raise FileNotFoundError(
            ...
            "Expected a config.yaml in the model directory."
        )
...
def _get_inp_pipeline(num_steps: int) -> tuple:
    global _pipeline_inp, _vae_inp, _config_inp
    if _pipeline_inp is None:
        logger.info(f"Loading InP pipeline from {INP_MODEL_NAME}")
        device = _get_device()
        model_path = os.path.join(LOCAL_MODELS_PATH, INP_MODEL_NAME)
        _config_inp = _load_config(model_path)

        transformer, transformer_2 = _load_transformers(model_path, _config_inp)
        _vae_inp = _load_vae(model_path, _config_inp)
        tokenizer, text_encoder = _load_text_components(model_path, _config_inp)
        scheduler = _load_scheduler(_config_inp)

        _pipeline_inp = Wan2_2FunInpaintPipeline(
        ...
        _apply_memory_mode(_pipeline_inp, transformer, transformer_2, device)
        _setup_teacache(_pipeline_inp, transformer_2, model_path, TEACACHE_THRESHOLD_T2V, num_steps)
        logger.info("InP pipeline ready")

    return _pipeline_inp, _vae_inp, _config_inp


def _get_ctrl_pipeline(num_steps: int) -> tuple:
    global _pipeline_ctrl, _vae_ctrl, _config_ctrl
    if _pipeline_ctrl is None:
        logger.info(f"Loading Control pipeline from {CTRL_MODEL_NAME}")
        device = _get_device()
        model_path = os.path.join(LOCAL_MODELS_PATH, CTRL_MODEL_NAME)
        _config_ctrl = _load_config(model_path)

        transformer, transformer_2 = _load_transformers(model_path, _config_ctrl)
        _vae_ctrl = _load_vae(model_path, _config_ctrl)
        tokenizer, text_encoder = _load_text_components(model_path, _config_ctrl)
        scheduler = _load_scheduler(_config_ctrl)

        _pipeline_ctrl = Wan2_2FunControlPipeline(
            transformer=transformer,
            transformer_2=transformer_2,
            vae=_vae_ctrl,
            tokenizer=tokenizer,
        ...
        _apply_memory_mode(_pipeline_ctrl, transformer, transformer_2, device)
        _setup_teacache(_pipeline_ctrl, transformer_2, model_path, TEACACHE_THRESHOLD_CTRL, num_steps)
        logger.info("Control pipeline ready")

    return _pipeline_ctrl, _vae_ctrl, _config_ctrl
...
def run_inference(job: Job, output_path: str) -> None:
    ...
    video_paths = [p for p in media_paths if p.endswith(".mp4")]

    if not media_paths:
        mode = "t2v"
    elif image_paths and not video_paths:
        mode = "i2v"
    elif image_paths and video_paths:
        mode = "v2v"
    else:
        raise ValueError(f"Cannot determine inference mode from mediaPaths: {media_paths}")

    video_length_raw = infer.videoLength
    fps = infer.fps
    shift = infer.shift

    logger.info(f"Mode: {mode} | size={sample_size} | steps={infer.numSteps} | seed={seed} | frames={video_length_raw} | fps={fps}")

    if mode in ("t2v", "i2v"):
        pipeline, vae, config = _get_inp_pipeline(infer.numSteps)
        boundary = config["transformer_additional_kwargs"].get("boundary", 0.900)
        transformer_2 = pipeline.transformer_2
        video_length = _aligned_video_length(video_length_raw, vae.config.temporal_compression_ratio)
        ...
        pipeline, vae, config = _get_ctrl_pipeline(infer.numSteps)
        ...
        control_video, _, _, _ = get_video_to_video_latent(
            local_video_path, video_length=video_length, sample_size=sample_size, fps=fps,
        )
