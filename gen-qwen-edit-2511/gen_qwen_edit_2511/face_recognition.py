...
class _FaceRecognitionInstance:
    ...
    def load(self):
        ...
        self._face_app.prepare(ctx_id=0, det_size=INSIGHTFACE_DET_SIZE)
    ...
    def _evict_embedding_cache(self, now: float) -> None:
        expired_keys = [k for k, (_, expiry) in self._embedding_cache.items() if now > expiry]
        for k in expired_keys:
            del self._embedding_cache[k]
        if expired_keys:
            logger.debug(f"[instance {self.idx}] Evicted {len(expired_keys)} expired embedding cache entries ({len(self._embedding_cache)} remaining)")

        while len(self._embedding_cache) >= EMBEDDING_CACHE_MAX_SIZE:
            self._embedding_cache.popitem(last=False)
            logger.debug(f"[instance {self.idx}] Embedding cache full ({EMBEDDING_CACHE_MAX_SIZE}) — evicted oldest LRU entry")
    ...
    def get_face_embedding_cached(self, image_bytes: bytes) -> np.ndarray | None:
        ...
        logger.debug(f"[instance {self.idx}] Cache miss for embedding — computing face embedding")
    ...
    def calc_face_similarity(self, id_images_bytes: List[bytes], test_image_bytes: bytes) -> float | None:
        ...
        for index, id_image_bytes in enumerate(id_images_bytes):
            emb_id = self.get_face_embedding_cached(id_image_bytes)

            if emb_id is None:
                logger.warning(f"[instance {self.idx}] No face detected in one of the ID images, skipping")
                continue

            found_any        = True
            distance         = cosine(emb_id, emb_test)
            curr_similarity  = 1.0 - distance
            logger.debug(f"[instance {self.idx}] Face match {index}: {curr_similarity}")
    ...
    def check_face_match(self, img, id_photos: list) -> float:
        ...
        similarity     = self.calc_face_similarity(id_images_bytes, image_bytes)
        result         = round(float(similarity), 4) if similarity is not None else 0.0
        logger.info(f"[instance {self.idx}] Face match: {result}")
        return result
...
def acquire_face_recognition():
    """Lease one face recognition instance from the pool. Blocks until one is available."""
    inst = _pool.get()
    try:
        yield inst
    finally:
