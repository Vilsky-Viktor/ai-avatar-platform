...
export const generatePhotoSetCaptions = (parameters: AvatarParameters): string[] => {
  ...
  return [
    ...
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, neutral relaxed expression with closed mouth, ${isFemale ? 'white strapless top' : 'white sleeveless shirt'}, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, gentle smile with teeth showing, dark gray sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} front-right 45-degree quarter-view portrait, dark blue t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} front-left 45-degree quarter-view portrait, dark green t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} pure right-side 90-degree profile portrait, dark red t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} pure left-side 90-degree profile portrait, dark brown t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} 180-degree rear view portrait, only the back of the head and hair visible, gray concrete studio wall, soft diffused studio lighting`,
    `${AVATAR_REFERENCE_NAME} extreme tight portrait filling the entire frame, only head and hair with haircut visible, no neck or shoulders in frame, gray concrete studio wall, soft diffused studio lighting`,
    `${AVATAR_REFERENCE_NAME} full body upright standing pose, ${isFemale ? 'white strapless top and white running shorts' : 'white sleeveless shirt and white running shorts'}, barefoot on light wooden floor, gray concrete studio wall, even soft diffused studio lighting`,

    // --- Close-up emotions (orders 10–18) ---
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, downcast sorrowful expression, deep slate blue sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, intense furrowed brow expression, crimson red sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, mild upbeat relaxed expression, warm peach sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, broad open smile with teeth showing, bright yellow sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, wide-open startled expression, electric teal sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, tense drawn tight expression, burnt orange sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, reserved lowered withdrawn expression, soft rose sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, heavy-lidded drowsy relaxed expression, dusty lavender sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,
    `${AVATAR_REFERENCE_NAME} straight-on eye-level portrait, wide tense alert expression, dark violet sleeveless t-shirt, gray concrete studio wall, even soft diffused studio lighting, head and shoulders in frame`,

    // --- Front chest-up portraits (orders 19–24) ---
    `${AVATAR_REFERENCE_NAME} straight-on chest-up portrait, ${isFemale ? 'soft beige cashmere turtleneck' : 'olive green henley'}, cozy indoor living room setting, warm natural window light from the side`,
    `${AVATAR_REFERENCE_NAME} straight-on chest-up portrait, ${isFemale ? 'flowy sage green midi dress' : 'khaki jacket over white t-shirt'}, outdoor park setting, warm golden hour sunlight`,
    `${AVATAR_REFERENCE_NAME} straight-on chest-up portrait, ${isFemale ? 'silky emerald green camisole' : 'fitted black turtleneck with charcoal overcoat'}, city rooftop at night, warm tungsten ambient light`,
    `${AVATAR_REFERENCE_NAME} chest-up portrait with gaze directed upward, ${isFemale ? 'chunky cream cable-knit sweater' : 'soft taupe merino crewneck'}, cozy indoor living room setting, warm candlelight`,
    `${AVATAR_REFERENCE_NAME} chest-up portrait with direct forward stare, ${isFemale ? 'tailored pale blue Oxford shirt' : 'light gray button-down shirt with light blue tie'}, modern office interior, bright overhead white lighting`,
    `${AVATAR_REFERENCE_NAME} straight-on chest-up portrait, fitted dark charcoal turtleneck, deep dark studio environment, soft low-key dramatic side lighting`,

    // --- Quarter chest-up portraits (orders 25–26) ---
    `${AVATAR_REFERENCE_NAME} front-right 45-degree quarter-view chest-up portrait, ${isFemale ? 'oversized oatmeal cardigan' : 'soft gray zip-up hoodie'}, loft-style living room interior, warm indoor lamp light`,
    `${AVATAR_REFERENCE_NAME} front-left 45-degree quarter-view chest-up portrait, ${isFemale ? 'beige trench coat over cream turtleneck' : 'camel overcoat over white crewneck'}, indoor cafe setting, soft natural daylight`,

    // --- Side chest-up portraits (orders 27–28) ---
    `${AVATAR_REFERENCE_NAME} right-side profile chest-up portrait, ${isFemale ? 'light pink windbreaker jacket' : 'light blue windbreaker jacket'}, urban city street background, early evening ambient light`,
    `${AVATAR_REFERENCE_NAME} left-side profile chest-up portrait, ${isFemale ? 'metallic silver cropped jacket' : 'black puffer vest over gray hoodie'}, urban city street background, early evening ambient light`,

    // --- Upper body / waist-up (orders 29–30) ---
    `${AVATAR_REFERENCE_NAME} waist-up portrait sitting at a desk, ${isFemale ? 'soft gray tailored blazer' : 'slim-fit navy blazer with white dress shirt'}, modern co-working office space, bright overhead office lighting`,
    `${AVATAR_REFERENCE_NAME} waist-up portrait standing upright, ${isFemale ? 'black racing-style crop jacket' : 'black racing jacket with gray t-shirt'}, luxury car garage interior, even overhead indoor lighting`,

    // --- Full body diverse poses (orders 31–43) ---
    `${AVATAR_REFERENCE_NAME} full body walking forward, ${isFemale ? 'black cocktail dress with high heels' : 'charcoal suit with white dress shirt'}, city sidewalk, overcast diffused daylight`,
    `${AVATAR_REFERENCE_NAME} full body in a deep squat pose, ${isFemale ? 'taupe linen wide-leg trousers and olive linen top' : 'taupe linen trousers and olive linen shirt'}, outdoor nature setting, soft diffused daylight`,
    `${AVATAR_REFERENCE_NAME} full body sitting on a hotel bed, ${isFemale ? 'pink racerback crop tank and gray fleece shorts' : 'relaxed gray joggers and white t-shirt'}, hotel room interior, soft ambient light`,
    `${AVATAR_REFERENCE_NAME} full body standing barefoot, ${isFemale ? 'breezy floral sundress' : 'red shorts and sleeveless black t-shirt'}, sandy beach with ocean in background, warm golden hour sunlight`,
    `${AVATAR_REFERENCE_NAME} full body sitting on a lounge chair, ${isFemale ? 'yellow floral wrap midi dress' : 'pastel yellow shorts and hawaiian shirt'}, beach setting, soft natural daylight`,
    `${AVATAR_REFERENCE_NAME} full body jogging on a path, ${isFemale ? 'breezy white cover-up dress' : 'linen shorts and sleeveless white t-shirt'}, tropical path with greenery, soft natural daylight`,
    `${AVATAR_REFERENCE_NAME} full body sitting cross-legged on the ground, ${isFemale ? 'terracotta boho pants and cropped top' : 'dark green cargo shorts and gray t-shirt'}, park grass setting, soft early morning light`,
    `${AVATAR_REFERENCE_NAME} full body leaning against gym lockers, ${isFemale ? 'all-black athleisure set' : 'all-black tracksuit'}, gym changing room interior, even overhead indoor lighting`,
    `${AVATAR_REFERENCE_NAME} full body lying on abdomen on a bed, ${isFemale ? 'soft pink sleep pajama' : 'gray modal lounge shorts and t-shirt'}, bedroom interior, soft warm romantic light`,
    `${AVATAR_REFERENCE_NAME} full body dancing on a street at night, ${isFemale ? 'black micro mini skirt and sleeveless white bodysuit' : 'black crew-neck t-shirt and black trousers'}, city street with colorful lights in background`,
    `${AVATAR_REFERENCE_NAME} full body extreme low-angle upward shot, blue jeans and light gray polo shirt, white clouds and pebbled ground in background, soft natural daylight`,
    `${AVATAR_REFERENCE_NAME} full body extreme high-angle overhead downward shot, white shorts and light gray t-shirt with orange flip flops, sahara desert terrain, bright sunny daylight`,
    `${AVATAR_REFERENCE_NAME} full body rear view standing on a balcony, ${isFemale ? 'high-waisted black trousers and white crop top' : 'slim navy chinos and white oxford shirt'}, city skyline background, natural daylight`,
  ];
};
